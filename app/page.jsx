          ...
          {a.image_url ? (
            <>
              <img
                ...
                style={{ filter: "blur(20px) brightness(0.4)", transform: "scale(1.3)" }}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center" onClick={onClose}>
      ...
      <div className="relative w-full max-w-sm rounded-t-3xl overflow-hidden"
          ...
          {anime.image_url ? (
            <>
              <img src={anime.image_url} alt=""
                className="absolute inset-0 w-full h-full object-cover"
                style={{ filter: "blur(18px) brightness(0.4)", transform: "scale(1.3)" }}
                onError={e => { e.target.style.display = "none"; }} />
              <img src={anime.image_url} alt={anime.title}
                className="absolute inset-0 w-full h-full object-contain"
                onError={e => { e.target.style.display = "none"; }} />
        ...
        {/* buttons */}
        <div className="flex flex-col gap-2 px-5 pb-8 pt-1">
          {(anime.buttons && anime.buttons.length > 0) ? (
            anime.buttons.map((btn, i) => (
              <a key={i} href={btn.link || "#"} target="_blank" rel="noopener noreferrer"
                className="w-full py-3 rounded-2xl text-sm font-black text-white text-center active:scale-95 transition-transform flex items-center justify-center gap-2"
                style={{ background: i === 0
                  ? "linear-gradient(135deg,#00B4FF,#0077BB)"
                  : i === 1
          ...
          <button onClick={onClose}
            className="w-full py-2.5 rounded-2xl text-sm font-bold text-gray-500 mt-1"
...
export default function Home() {
  ...
  // featured anime slider (max 7), fallback to top 5 by views
  const featuredAnime = anime.filter(a => a.featured);
  const top5 = featuredAnime.length > 0
